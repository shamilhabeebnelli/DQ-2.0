# © By WhiteEye™ And IndianBot™
# For WhiteEyeUserBot And Indianbot ™
...
async def download(target_file):
    ...
    if "|" in input_str:
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff % 10.00) == 0 and current_message != display_message:
                    await WhiteEye.edit(current_message)
        ...
        if downloader.isSuccessful():
            ...
            await WhiteEye.edit("Incorrect URL\n{}".format(url))
        ...
        try:
            ...
            await WhiteEye.edit(str(e))
        else:
            await WhiteEye.edit(
                "Downloaded to `{}` successfully !!".format(downloaded_file_name)
            )
    else:
        await WhiteEye.edit("Reply to a message to download to my local server.")
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the WhiteEyeUserBot's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("WhiteEyeUserBot.session", "config.env"):
