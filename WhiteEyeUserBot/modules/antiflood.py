...
import WhiteEyeUserBot.plugins.sql_helper.antiflood_sql as sql
import WhiteEyeUserBot.utils
from WhiteEyeUserBot.utils import humanbytes, progress, time_formatter
...
@WhiteEye.on(WhiteEye_on_cmd(incoming=True))
async def _(event):
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="setflood (.*)"))
async def _(event):
    if event.fwd_from:
        return
