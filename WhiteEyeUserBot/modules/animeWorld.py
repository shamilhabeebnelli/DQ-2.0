...
@WhiteEye.on(WhiteEye_on_cmd(pattern="ainfo ?(.*)"))
@WhiteEye.on(sudo_cmd(pattern="ainfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="manga ?(.*)"))
@WhiteEye.on(sudo_cmd(pattern="manga ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
