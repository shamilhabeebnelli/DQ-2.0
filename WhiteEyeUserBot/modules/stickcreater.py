...
@WhiteEye.on(WhiteEye_on_cmd(pattern="stcr ?(?:(.*?) \| )?(.*)", outgoing=True))
async def sticklet(event):
    ...
    if event.reply_to_msg_id:
        reply_message = await event.get_reply_message()
    # delete the WhiteEyeUserBot command,
    ...
    image_stream.name = "@WhiteEye.webp"
...
CMD_HELP.update(
    {
        "stickcreater": "StickerCreater\
\n\nSyntax : .stcr <text>\
\nUsage : Converts The Text Into Sticker"
    }
)
