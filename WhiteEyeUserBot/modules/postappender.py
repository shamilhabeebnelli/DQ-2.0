...
from WhiteEyeUserBot.modules.sql_helper.PostAppender_sql import (
    add_new_datas_in_db,
    is_data_indbs,
    is_footer,
    remove_dataz,
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="spf (.*)"))
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="sph (.*)"))
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="rpd$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="spad$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
    if is_data_indbs(id_s):
