...
@WhiteEye.on(WhiteEye_on_cmd(pattern="kang ?(.*)"))
@WhiteEye.on(sudo_cmd(pattern="kang ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    moods = await edit_or_reply(
        event, "`Hello, This Sticker Looks Noice. Mind if I steal it. Dil Chori Sada Ho Gya, Sticker Tera Chori Ho Gya`"
    )
    ...
    if is_a_s:
        ...
        if userid == 813878981:
            ...
            packname = f"@{user.username} KangPack {pack}"
            packshortname = f"WhiteEye_{userid}"  # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    # if userid == 948408212:
                    # packname = f"{user.username}'s {pack}"
                    # packshortname = "Vol._{pack}_WhiteEye_ke_locker_me"
                    # else:
                    # packname = f"Vol._{pack}_WhiteEye{userid}"
                    # packshortname = f"Vol._{pack}_WhiteEye_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
                        await moods.edit(
                            "**Pack No. **"
                            + str(prevv)
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="packinfo"))
@WhiteEye.on(sudo_cmd(pattern="packinfo ?(.*)", allow_sudo=True))
async def _(event):
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="getsticker ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
